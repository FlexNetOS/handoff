...
fn cmd_seed() {
    ...
    let backlog = vec![
        ...
        mk("HFTASK-0007", "hf session via meta git worktree + policy.toml + sync preflight", Priority::P0,
           "ADR-0001 §2: add `hf session start|end [--recycle]` using `meta git worktree create|remove|prune` (NOT raw git worktree) off origin/<base_branch>, reserve a weave path-scope lease, emit session_start/session_end; recycle a fresh set on end. Add `.handoff/policy.toml` (remote/loop/merge). MUST include a start-time preflight that verifies tree/branch/remote are in sync and refuses on drift (lesson from the prior weave loop failure). Depend only on the current lease-capable weave; refuse legacy repowire/mcp-broker.", &["HFTASK-0002"]),
        ...
        mk("HFTASK-0010", "PR review/merge automation - phased cloud_ultra->swarm_local + permission gate", Priority::P1,
           "ADR-0001 §5: reviewer is always a separate role, filled by merge.reviewer. Phase 1 cloud_ultra: `hf review request <pr#>` runs /code-review ultra on the PR, records approve/deny via weave review (WL-020) + permission ask (WL-021). Phase 2 swarm_local: replace with ruvector/ruflo agent-swarm reviewer. `hf merge <pr#>` merges only on approved+permission-granted (gh pr merge --squash), ff develop; deny re-opens task; pending waits (retryable). permission_gate is transitional (lift once swarm trusted). Target current-weave queues only.", &["HFTASK-0009"]),
