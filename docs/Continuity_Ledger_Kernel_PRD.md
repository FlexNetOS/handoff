---
title: "Continuity Ledger Kernel - PRD"
...
# Continuity Ledger Kernel - PRD

## 0. Design Summary

The Continuity Ledger Kernel is a Rust-native, repo-local handoff kernel for AI coding agents: a CLI, task ledger, session journal, repo map, path leases, worktrees, hooks, MCP bridge, skills, sub-agents, swarms, and tests.

It is specified as a strict implementation contract, not a narrative. The design commits to the following surfaces:

| Area | Design commitment |
|---|---|
| Source of truth | Explicit state precedence: Git + ledger + task cards are authoritative; handoff packets are compiled views |
| Drift control | Intent locks, goal hashes, task scope hashes, acceptance signatures, drift audit gates, and evidence-backed completion |
| Conflict control | SQLite WAL, atomic lease transitions, file locks, stale lease recovery, and merge serialization |
| Clarity | Separates product contract, repo contract, state machine, command contract, and task backlog |
| Sub-agents | Agent classes, allowed operations, write scope, failure behavior, and swarm admission control |
| Hooks | Enforceable hook input/output contract and must-pass gates |
| Tests | Replay tests, drift tests, crash tests, concurrent write tests, golden handoff packet tests, and fresh-agent acceptance tests |
| Repo navigation | Context capsule, navigation index, decision records, glossary, command matrix, and ownership surfaces |
| Deliverability | Templates, schemas, task cards, policy files, and package layout |
...
Build the **Continuity Ledger Kernel**: a Rust-native, repo-local handoff kernel for AI coding agents.
...
This is the most demanding part of the contract.
...
```json
{
  ...
  "project_name": "Continuity Ledger Kernel",
